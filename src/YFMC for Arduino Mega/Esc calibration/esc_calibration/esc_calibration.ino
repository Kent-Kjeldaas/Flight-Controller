...
void setup(){  
  ...
  DDRA |= B11110000;                                 //Configure digital poort 26, 27, 28 and 29 as output(esc)
 // DDRB |= B00010000;                                 //Configure digital poort 12 as output
 
  PCICR |= (1 << PCIE0);                             // set PCIE0 to enable PCMSK0 scan
  PCMSK0 |= (1 << PCINT0);                           // set PCINT0 (digital input 8) to trigger an interrupt on state change 53
  PCMSK0 |= (1 << PCINT1);                           // set PCINT1 (digital input 9)to trigger an interrupt on state change 52
  PCMSK0 |= (1 << PCINT2);                           // set PCINT2 (digital input 10)to trigger an interrupt on state change 51
  PCMSK0 |= (1 << PCINT3);                           // set PCINT3 (digital input 11)to trigger an interrupt on state change 50
...
void loop(){
  ...
  PORTA |= B11110000;                                        //Set port 4, 5, 6 and 7 high at once ( 29, 28, 27, 26)
  ...
  while(PORTA >= 16){                                        //Execute the loop until digital port 4 to 7 is low
    ...
    if(timer_channel_1 <= esc_loop_timer)PORTA &= B11101111; //When the delay time is expired, digital port 4 is set low
    if(timer_channel_2 <= esc_loop_timer)PORTA &= B11011111; //When the delay time is expired, digital port 5 is set low
...
ISR(PCINT0_vect){
  current_time = micros();
  //Channel 1=========================================
  if(PINB & B00000001){                                        //Is input 8 high? 53
